...
const workspaceManifestPath = resolveWorkspaceManifestPath(process.env);
...
async function run() {
  ...
  assert(goal.persistence && goal.persistence.files.length === 5, 'dynamic state artifacts 持久化失败');
